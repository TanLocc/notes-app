      ...
      steps {
          script{
            withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentail', accessKeyVariable: 'AWS_ACCESS_KEY_ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
              ...
              sh """
                aws cloudformation deploy  --template-file build-node.yml  --stack-name build-node --parameter-overrides \
                VpcId=${VPC_ID} ClusterControlPlaneSecurityGrou=${security_Group} ClusterName=capstone KeyName=micro Subnets=${subnets} NodeGroupName=groupWorker --capabilities CAPABILITY_NAMED_IAM
              """
